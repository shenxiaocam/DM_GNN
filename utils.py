...
def entropy_compute(input_):
    epsilon = tf.constant(1e-10)
    entropy= - tf.math.multiply(input_, tf.log(input_ + epsilon))
...
#xs_ys_batch_csr, shuffle_index = next(S_batches)
#xs_batch_csr=xs_ys_batch_csr[0]
#ys_batch =xs_ys_batch_csr[1]

        



def fc_layer(input_tensor, input_dim, output_dim, layer_name, act=tf.nn.relu, input_type='dense',drop=0.):
    with tf.name_scope(layer_name):
        ...
        if input_type == 'sparse':            
            ...
            input_tensor = tf.nn.dropout(input_tensor, rate=drop)
...
def ComputePPMI(A):
    ...
    ### PMI, each row of A divided by col
...
def batchPPMI(batch_size,shuffle_index_s,shuffle_index_t,PPMI_s,PPMI_t):
    ...
    #proximity matrix between target network nodes in the batch
